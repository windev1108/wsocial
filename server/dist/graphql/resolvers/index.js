...
const resolvers = {
    Query: {
        getUsers: async (_parent, _args, { prisma }) => {
        ...
        getPosts: async (_parent, { userId }, { prisma }) => {
        ...
        getPostById: async (_parent, args, { prisma }) => await prisma.post.findUnique({
            where: {
                id: args.id,
            },
            include: {
    ...
    User: {
        posts: async (_parent, { isMySelf, isFriend }, { prisma }) => {
            if (isMySelf) {
                const posts = await prisma.post.findMany({
                    where: {
                        authorId: _parent.id,
        ...
        friends: async (_parent, _args, { prisma }) => {
            const user = await prisma.user.findUnique({
                where: { id: _parent.id },
                select: { friends: true },
            });
        ...
        followers: async (_parent, _args, { prisma }) => {
            const user = await prisma.user.findUnique({
                where: { id: _parent.id },
        ...
        followings: async (_parent, _args, { prisma }) => {
            const user = await prisma.user.findUnique({
                where: { id: _parent.id },
    ...
    Post: {
        author: async (_parent, _args, { prisma }) => await prisma.user.findUnique({
            where: {
                id: _parent.authorId,
            },
        }),
        likes: async (_parent, _args, { prisma }) => {
        ...
        files: async (_parent, _args, { prisma }) => await prisma.file.findMany({
            where: {
                postId: _parent.id,
            },
        }),
